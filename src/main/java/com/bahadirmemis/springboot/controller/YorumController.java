...
public class YorumController {
    ...
    public MappingJacksonValue findAllYorumByKullaniciId (@PathVariable Long id) {
        ...
        List<YorumDto> list = YorumConverter.INSTANCE.convertYorumListToYorumDtoList(yorumEntityService.findAllYorumByKullaniciId(id));
        if(list.isEmpty()) {
    ...
    public MappingJacksonValue findAllYorumByUrunId (@PathVariable Long id) {
        ...
        List<YorumDto> list = YorumConverter.INSTANCE.convertYorumListToYorumDtoList(yorumEntityService.findAllYorumByUrunId(id));
        if(list.isEmpty()) {
    ...
    public ResponseEntity<Object> saveYorum(@RequestBody YorumDto yorumDto) {
        Yorum yorum = YorumConverter.INSTANCE.convertYorumDtoToYorum(yorumDto);
        yorum = yorumEntityService.save(yorum);

        URI uri = ServletUriComponentsBuilder
                .fromCurrentRequest()
                .path("{id}")
                .buildAndExpand(yorum.getId())
                .toUri();
